...
<template>
  <div
    class="bg-white rounded border border-[#DFE1E6] border-l-[3px] p-3 cursor-pointer hover:bg-[#E4F0FF] transition-colors group relative"
    :class="priorityBorder[task.priority]"
    @click="emit('edit', task)"
  >
    <button
      class="absolute top-2 right-2 p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-[#FFEBE6] text-[#6B778C] hover:text-[#DE350B] transition-all"
      @click.stop="emit('delete', task.id)"
    >
      <Trash2 class="w-3.5 h-3.5" />
    </button>

    <h4 class="text-sm font-medium text-[#172B4D] pr-6 leading-5">{{ task.title }}</h4>
