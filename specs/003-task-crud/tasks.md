...
- [ ] T001 登记 `specs/003-task-crud/` 四件套并提交
...
- [ ] T002 [P] [Red] `src-tauri/src/models.rs` 尚不存在 — 先写
      ...
      `cargo test` → RED。
- [ ] T003 [Green] `error/mod.rs` 增加 `Validation(String)` variant。
      `cargo test` → GREEN。
- [ ] T004 [P] [Red] 写 `src-tauri/src/repo/tasks_tests.rs`:
      `test_insert_and_list_orders_by_created_desc` — tempfile DB,
      插入 3 条 (created_at 递增), 断言 list 顺序 + 全字段回读一致。
      `cargo test` → RED (模块不存在)。
- [ ] T005 [Green] `src-tauri/src/models.rs` (`Task` 结构体 +
...
- [ ] T006 [P] [Red] repo 测试: `test_update_partial_fields` —
...
- [ ] T007 [Green] repo: `update_task`(动态 SET), `set_task_status`,
      `delete_task`。`cargo test` → GREEN。
- [ ] T008 [P] [Red] repo 测试: `test_list_1000_rows_under_50ms` —
      插入 1,000 行, 计时 `list_tasks` < 50ms (SC-001)。RED (如未
      优化自然失败则记录基线数字)。
- [ ] T009 [Green] 若 T008 红: 加索引评估 (created_at 已是 TEXT,
      必要时 `CREATE INDEX` 走 v2 migration — **需回炉 spec**, 见
      FR-015)。预期 GREEN 无需改动。
- [ ] T010 `cargo clippy --all-targets -- -D warnings` clean。
...
- [ ] T011 [P] [Red] 写 `src-tauri/src/commands/tasks_tests.rs` 校验
      ...
      `TaskNotFound`。RED。
- [ ] T012 [Green] 实现 `src-tauri/src/commands/tasks.rs`:
...
- [ ] T013 [Green] `chrono` 依赖入 `Cargo.toml`
      (`features = ["serde"]`);时间戳统一
      `to_rfc3339_opts(SecondsFormat::Secs, true)`。
- [ ] T014 `cargo clippy` clean + 全套 `cargo test` 回归
      (001 的 8 个测试不回归)。
...
- [ ] T020 [P] [Red] `src/__tests__/ListView.test.tsx`: mock ipc →
      `listTasks` 返回 3 条 → 渲染 3 行倒序; 每行显示标题/状态/优先级;
      due_at 有 → 显示日期, 无 → 不显示; 空数组 → 空状态文案
      (`tasks.empty`)。RED。
- [ ] T021 [Green] 实现 `src/views/ListView.tsx` (替换占位) +
      ...
      `pnpm test` → GREEN。
- [ ] T022 App 集成: ListView 挂 `onOpenSettings` 之外的
...
- [ ] T023 [P] [Red] `TaskRow.test.tsx`: 点击状态控件 → 调
      `setTaskStatus(id, next)` 且顺序 todo→doing→done→todo;
      编辑/删除按钮触发回调。RED。
- [ ] T024 [Green] TaskRow 接线状态循环 + 回调 props。GREEN。
- [ ] T025 [P] [Red] ListView 集成测试: 新建对话框提交 →
...
- [ ] T026 [Green] ListView 接线三个对话框流 (D7: 删除确认无需
...
- [ ] T027 `.specify/memory/design.md` 新增 **Section 6.7**:
...
- [ ] T028 `pnpm test:visual` → 0 drift (amendment 不破坏既有 token);
...
- [ ] T029 i18n 审计: `pnpm check:i18n` 0 违规 (新 key 全在
      zh-CN.ts)。
- [ ] T030 `tsc --noEmit` / `pnpm test` / `cargo test` /
